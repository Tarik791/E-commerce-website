...
<?php if(is_object($profile_data)): ?>
	...
	<?php if(is_array($orders)):?>
			...
			<table class="table">
				<thead>
					<tr><th>Order no</th><th>Order date</th><th>Total</th><th>Check-out date</th><th>City/State</th><th>Mobile Phone</th><th>Status</th><th>...</th></tr>
				</thead>
				<tbody onclick="show_details(event)">
					<?php foreach($orders as $order):?>
						...
						?>
						<tr style="position: relative;"><td><?=$order->id?></td><td><?=date("jS M Y H:i a",strtotime($order->date))?></td><td>$<?=$order->total?></td><td><?=$order->datetimetwo?></td><td><?=$order->state?></td><td><?=$order->mobile_phone?><td><?=$status?></td>
							<td>
